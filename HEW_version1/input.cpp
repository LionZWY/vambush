...
// マウス
static LPDIRECTINPUTDEVICE8 pMouse = NULL;									// マウス
...
static DWORD	padTrigger[GAMEPADMAX];										// パッド情報
...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		// 右スティク
		// 上下
		if (dijs.lRz < 0)					padState[i] |= R_BUTTON_UP;
		//* y-axis (backward)
		if (dijs.lRz > 0)					padState[i] |= R_BUTTON_DOWN;

		// 左右
		//* x-axis (left)
		if (dijs.lZ < 0)					padState[i] |= R_BUTTON_LEFT;
		//* x-axis (right)
		if (dijs.lZ > 0)					padState[i] |= R_BUTTON_RIGHT;
